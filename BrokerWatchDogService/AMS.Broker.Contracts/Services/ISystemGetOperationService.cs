...
namespace AMS.Broker.Contracts.Services
{
    ...
    public interface ISystemGetOperationService
    {
        ...
        [OperationContract]
        IList<DeviceDto> GetUserDevices(string userID, string searchString, int _ModuleFlag, string accountId, string siteId, string areaId, string floorId, string zoneId, string InterfaceType, string InterfaceSubType, string Interface, string authToken, bool IsNewUser); //for touch config user privilage list
        ...
        [OperationContract]
        DeviceDto GetSingleDeviceDetailsConfig(string authToken, long deviceId); //trupti090818

        [OperationContract]
        IEnumerable<ObjectsAssociationDto> GetAssociatedDeviceList(int DeviceId, string authToken);//trupti090818

        [OperationContract]
        IEnumerable<SP_ReturnUserDeviceNonAssignedList_ResultDTO> GetAssignDeviceUserList(string ParentDeviceId, string AssoDeviceId, string authToken);

        [OperationContract]
        IEnumerable<SP_ReturnUserObjectNonAssociationList_ResultDTO> GetUserAssociatedDeviceDataForUpdate(string DeviceList, string authToken, string UserId);

        [OperationContract]
        IEnumerable<SP_ReturnDeviceObjectAssociationList_ResultDTO> GetUserAssociatedDeviceData(string DeviceList, string authToken);

        [OperationContract]
        IEnumerable<SP_AllInterfaceDeviceStatus_ResultDTO> GetAllInterfaceDeviceStatusData(string authToken);  //manish19112019

        [OperationContract]
        IEnumerable<tblInterfaceOperationsDTO> GetInterfaceOperations(string authToken);

        [OperationContract]
        IEnumerable<tblInterfaceOperationsMasterDTO> GetInterfaceOperationMasters(string authToken);

        [OperationContract]
        List<InterfaceDto> GetAllInterfacesForUsers(string authToken);


        [OperationContract]
        IEnumerable<tblAnalyticsInfoDto> GetMultiAnalyticsList(int DeviceID, string authToken); //Tejasvini 15012019tttt
