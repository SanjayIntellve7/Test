...
namespace AMS.Broker.WatchDogService
{
    public class BrokerService : ServiceBase
    {
        ...
        protected override void OnStart(string[] args)
        {
            ...
            {             
                ...
                if (IsStartBroker == true)
                {
                    ...
                    //CameraStatusService.Initialise();
                    var _isInitialiseFail = false;

                    ServerClientCommunicationService.Initialise(this);//amit 03052018
                 
                    //_IcameraStatus.StartCameraStatusTimer();
                    DeviceTimeSyncService.Initialise(this);               

                    EscallationInformService.Initialise(this);

                    WatchDogMonitorService.Initialise(this);

                    if (isInitialiseFail)
                    {
                        InsertBrokerOperationLog.AddProcessLog("Watchdog Service initialise fail.Hence Stop the service");
        ...
        public static IEnumerable<tblSMTPConfigurationDTO> GetSMTPConfiguration()
        {
            try
            {
                using (var context = new CentralDBEntities())
                {
                    List<tblSMTPConfigurationDTO> smtpConfig = new List<tblSMTPConfigurationDTO>();
                    foreach (var _smtpConfig in context.tblSMTPConfiguration)
                    {
                        tblSMTPConfigurationDTO smtpconfig = new tblSMTPConfigurationDTO()
                        {
                            ID = _smtpConfig.ID,
                            Name = _smtpConfig.Name,
                            SMTPServer = _smtpConfig.SMTPServer,
                            SMTPPort = _smtpConfig.SMTPPort,
                            SMTPUser = _smtpConfig.SMTPUser,
                            SMTPPassword = _smtpConfig.SMTPPassword,
                            UseAuthentication = _smtpConfig.UseAuthentication,
                            UseSSL = _smtpConfig.UseSSL,
                            EmailDisplayName = _smtpConfig.EmailDisplayName,
                            FromAddress = _smtpConfig.FromAddress,
                        };
                        smtpConfig.Add(smtpconfig);
                    }
            ...
            {
                _logger.Info("LookupServiceImpl GetSMTPConfiguration() Exception:" + ex.Message);
                string Message = "LookupServiceImpl GetSMTPConfiguration() Exception:" + ex.Message;                
        ...
        private static void SendMail(string ServiceName)
        {
            try
            {
                string exitEmail = Storage.ServiceExitEmail;
                var emailId = exitEmail.Split(',');
                string ExitMessage = ServiceName + " did not initialised successfully." + " Hence service will be stopped.";
                foreach (var email in emailId)
                {
                    var trimmedEmail = email.Trim();

                    if (ExitMessage != null && ExitMessage != "" && trimmedEmail != null && trimmedEmail != "")
                    {
                        string Message = ServiceName + " -SendMail() -- Message = " + ExitMessage + "--!--Receiver:" + trimmedEmail;
                        InsertBrokerOperationLog.AddProcessLog(Message);
                        ...
                        foreach (var smtpconfig in smtpConfiguration)
                        {
                            ...
                            string fromEmail = smtpconfig.FromAddress;
                            string mailid = trimmedEmail;
                            string SMTPServer = smtpconfig.SMTPServer;
                            string SMTPPort = smtpconfig.SMTPPort;
                            string SMTPPass = smtpconfig.SMTPPassword;
                            string SMTPUser = smtpconfig.SMTPUser;
                            bool UseAuthentication = (bool)smtpconfig.UseAuthentication;
                            bool UseSSL = (bool)smtpconfig.UseSSL;
                            ...
                            if (SetMailAddressCollection(message.To, mailid))
                            {
                                try
                                {
                                    ...
                                    message.Body = ExitMessage;
                                    message.IsBodyHtml = true;

                                    emailClient.Host = SMTPServer;
                                    emailClient.Port = Int32.Parse(SMTPPort.ToString());
                            ...
                            if (message != null) // jatin 13022019
                            {
                                message.Dispose();
                                message = null;
                            }

                            if (emailClient != null)// jatin 13022019
                            {
                                emailClient.Dispose();
                                emailClient = null;
                            }
                    ...
                    {
                        string Message = ServiceName + " -SendMail() -- Email not found";
                        //InsertExtNotificationLog.AddProcessLog(Messsg);
                        InsertBrokerOperationLog.AddProcessLog(Message);
                    }
                }
            }
            ...
            {
                string Message = ServiceName + " -SendMail() -- Exception = " + ex.Message;
                InsertBrokerOperationLog.AddProcessLog(Message);
            }
        }
