...
using AMS.Broker.IntegrationService.Services;
using NLog;
...
namespace AMS.Broker.IntegrationService
{
    public class BrokerService : ServiceBase
    {
        ...
        public  NVRServiceAct _nvrService = null;
        public ServiceHost serviceHost = null;
        public BrokerMembershipProvider brokerMemProvider;  //amit 18052017

        public BrokerService()
        {
            this.ServiceName = "BrokerIntegrationService";

            System.IO.Directory.SetCurrentDirectory(System.AppDomain.CurrentDomain.BaseDirectory);

            Console.WriteLine(" In Broker inside BrokerIntegrationService");
            InsertIntegrationLog.AddProcessLogIntegration(" In Broker inside BrokerIntegrationService");//jatin
        ...
        public void InitializeBrokerProvider()  //amit 18052017
        {
            try
            {
                ...
                ObjNameValueCollMembership.Add("applicationName", "AMS.Broker.IntegrationService");
        ...
        protected override void OnStart(string[] args)
        {
            try
            {
                ...
                if (Storage.IsLicenceCheck == "1")
                {
                    ...
                    RootDir = RootDir.Replace("AMS.Broker.IntegrationService.exe", "");
                    ...
                    if (Directory.Exists(RootDir))
                    {
                        ...
                        if (directory.GetFiles().Length > 0)
                        {
                            ...
                            if (filePaths.Count() > 0)
                            {
                                    ...
                                    if (ReqData.Count() == 9) // jatin 07112017 // jatin 08112017
                                    {
                                        ...
                                        if (ActivationGUID == SuccessGUID)
                                        {
                                            ...
                                            else
                                            {
                                                ...
                                                InsertIntegrationLog.AddProcessLogIntegration("Error while validating license, Please contact 2020imaging");//jatin
                                            }
                                        }
                                        else
                                        {
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    InsertIntegrationLog.AddProcessLogIntegration("Exception while validating license" + ex.Message);//jatin
                                }
                            }
                            else
                            {
                        ...
                        else
                        {
                            ...
                            InsertIntegrationLog.AddProcessLogIntegration("Licence file not found");//jatin
                        }
                    }
                    else
                    {
                        ...
                        InsertIntegrationLog.AddProcessLogIntegration("Licence Directory:" + RootDir + " :Not found");//jatin
                    }
                }
                else
                {
                 ...
                 if (IsStartBroker == true)
                 {
                     ...
                     {
                         _logger.Info("BrokerIntegrationService OnStart() : " + "Kindly install valid certificate from 2020Imaging!");
                         InsertIntegrationLog.AddProcessLogIntegration("BrokerIntegrationService OnStart() : " + "Kindly install valid certificate from 2020Imaging!");//jatin
                         Environment.Exit(1);
                     }
                     ...
                     CollectionContainer collectionContainerObj = new CollectionContainer();

                     DateTime midnight = DateTime.Now.Date;

                     string tempPath = System.IO.Path.GetTempPath();

                     var bizTalkHelper = new Helpers.BizTalkHelper();

                     _nvrService = new NVRServiceAct();
                     CameraControlService.Initialise(_nvrService);
                     // ACS Integration
                     var ACSIntegration = ACSIntegrationService.Initialise();
                     //ACSIntegration.TestMethod("Admin", "Admin");


                     AuthenticationHelper.LoadActiveDirectory();
                     //
                     //ModbusIntegration
                     // var ModbusIntegration = ModbusCommunicationService.Initialise(alertService, stationService, controllerService, authCode);
                     //ModbusIntegration.StartModbusDevicePoll();
                     /*ModbusCommunicationServiceImpl Modbus = new ModbusCommunicationServiceImpl(alertService, stationService, controllerService);
                     Modbus.CreateAlert(authCode, 10041);
                     Modbus.CreateAlert(authCode, 10045);
                     Modbus.CreateAlert(authCode, 10047);
                     Modbus.CreateAlert(authCode, 10048);
                     Modbus.CreateAlert(authCode, 10049);*/
                     //                
                     VideoAnalyticsManagerIntegrationService.Initialise();

                     //  HomeAutomationAlertData homeAlert = new HomeAutomationAlertData();
                     //   homeAlert = null;
                     //   homeAutomation.CreateHomeAutomationAlert(homeAlert);
                     //    DeviceData dev = new DeviceData();
                     //   dev.flag = 1;
                     //  homeAutomation.AddHomeAutomationDevice(dev);
                     MatrixControllerService.Initialise();

                     var IclockService = IclockACSIntegrationService.Initialise();
                   //  Mediaintegration _mediaInte = new Mediaintegration();
                   //  _mediaInte.CreateGetPhotoTask();
                    // var _CamTcpListenerService = CamTcpListenerService.Initialise();
                    // _CamTcpListenerService.StartListner();
                     base.OnStart(args);
                 }
            }
            catch (Exception e)
            {
                _logger.Info("BrokerIntegrationService OnStart() Exception:" + e.Message);
                InsertIntegrationLog.AddProcessLogIntegration("BrokerIntegrationService OnStart() Exception:" + e.Message);//jatin
                ExceptionHandler(e);
            }
        }
            ...
            try
            {
                ...
                foreach (var Dev in _deviceCollection)
                {
                    if (Dev is NvrCameraDto)
                    {
                        ...
                        {
                            _logger.Info("BrokerIntegrationService CreateXamlFile() Exception:" + ex.Message);
                            InsertIntegrationLog.AddProcessLogIntegration("BrokerIntegrationService CreateXamlFile() Exception:" + ex.Message);//jatin
                        }
