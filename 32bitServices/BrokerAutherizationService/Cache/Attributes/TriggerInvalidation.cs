    ...
    public static partial class Cache
    {
        ...
        public class TriggerInvalidation : OnMethodBoundaryAspect
        {
            ...
            public override void OnExit(MethodExecutionArgs args)
            {
                ...
                if (KeyBuilder.Settings == CacheSettings.IgnoreParameters)
                {
                    var keysToDelete = (from aKey in CacheService.Cache.Keys where aKey.Contains(KeyBuilder.GroupName) select key).Cast<object>().ToList();

                    foreach (var keytodelete in keysToDelete)
                    {
                        CacheService.Cache.Delete(keytodelete.ToString());
                    }
                }
