...
using AMS.Broker.AutherizationService.RihnoSecurity.Membership;
...
namespace AMS.Broker.AutherizationService.Helpers
{
    internal static class AuthenticationHelper
    {
        ...
        public static Guid LogInUser(this CentralDBEntities context, string sid, string identifier, string identifierIP, string component)
        {
            try
            {
                ...
                var _Gui = Guid.NewGuid();
                loginObject.AuthToken = GetStringFromIP(identifierIP, _Gui.ToString());//Guid.NewGuid();
                loginObject.Identifier = identifier;
                loginObject.LoginDate = DateTime.Now;
                loginObject.LogoutDate = null;
                loginObject.IdentifierIP = identifierIP; //amit 15112016 version 1.4.1
                loginObject.Component = component; //amit 15112016 version 1.4.1
        ...
        public static Guid GetStringFromIP(string IPAddr, string AuthToken) //trupti020217 for valid ip and auth user
        {
            try
            {
                var Spliter = IPAddr.Split('.');
                string _FirstPair = Spliter[0];
                if (_FirstPair.Length == 2)
                {
                    _FirstPair = "0" + _FirstPair;
                }
                if (_FirstPair.Length == 1)
                {
                    _FirstPair = "00" + _FirstPair;
                }

                string _secondPair = Spliter[1];
                if (_secondPair.Length == 2)
                {
                    _secondPair = "0" + _secondPair;
                }
                if (_secondPair.Length == 1)
                {
                    _secondPair = "00" + _secondPair;
                }

                string _thirdPair = Spliter[2];
                if (_thirdPair.Length == 2)
                {
                    _thirdPair = "0" + _thirdPair;
                }
                if (_thirdPair.Length == 1)
                {
                    _thirdPair = "00" + _thirdPair;
                }

                string _fourthPair = Spliter[3];
                if (_fourthPair.Length == 2)
                {
                    _fourthPair = "0" + _fourthPair;
                }
                if (_thirdPair.Length == 1)
                {
                    _fourthPair = "00" + _fourthPair;
                }

                string StringIP = _FirstPair + _secondPair + _thirdPair + _fourthPair;
                var _authToken = AuthToken.Substring(0, 24) + StringIP;
                Guid _authTok = new Guid(_authToken);
                return _authTok;
            }
            catch (Exception ex)
            { }
        ...
        public static string GetUsersSID(string username)
        {
            ...
            catch (System.Data.EntityException ee) // jatin 19012018
            {
                return "23";
            }
            catch (Exception ex)
        ...
        public static string GetMemUserID(string username)
        {
            try
            {
                using (var context = new CentralDBEntities())
                {
                    ...
                    if (user != null)
                        memuserid = user.MembershipUserId.ToString();
